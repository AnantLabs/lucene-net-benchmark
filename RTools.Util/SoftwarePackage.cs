...
namespace RTools.Util
{
	...
	public class SoftwarePackage
	{
		...
		public bool Uninstall(bool interactive)
		{
			...
			{
				// Process style
				Console.WriteLine("Process: {0}", cmdLine);
				p = new Process();
				p.StartInfo = psi;
				p.Start();
			...
			{
				// Executor style for arbitrary command line
		...
		public static SoftwarePackage[] GetList()
		{
			...
			if (key == null)
			{
				System.Console.WriteLine("Unable to open registry key {0}.", uninstallKey);
				return(null);
			}
		...
		{
			// list uninstallable packages
			Console.WriteLine("-----------------------------------------------------");
			Console.WriteLine("Uninstallable software packages:");
			SoftwarePackage[] setups = GetList();
			foreach(SoftwarePackage s in setups)
			{
				if (s.IsUninstallable) Console.WriteLine("setup: {0}", s);
			}

			// find a particular package
			Console.WriteLine("-----------------------------------------------------");
			string testPackageName = "SetupTest1";
			//string testPackageName = "GSview 4.3";

			SoftwarePackage tp = GetPackage(testPackageName);
			if (tp != null)
			{
				Console.WriteLine("Got package: {0}", tp.ToString());
			}
			else Console.WriteLine("Didn't find package {0}", testPackageName);

			// uninstall
			Console.WriteLine("-----------------------------------------------------");
			Console.WriteLine("Uninstall...");
			SoftwarePackage testSetup = FindOne(testPackageName, setups);
			if (testSetup != null)
			{
				testSetup.Uninstall(false);
			}
			else Console.WriteLine("Didn't find product {0} to uninstall it.", testPackageName);

			// uninstall all
//			Console.WriteLine("-----------------------------------------------------");
//			Console.WriteLine("UninstallMultiple...");
//			if (!UninstallMultiple(testPackageName, false))
//				Console.WriteLine("UninstallMultiple {0} failed.", testPackageName);
